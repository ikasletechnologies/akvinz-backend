...
exports.submitLocationChange = submitLocationChange;
exports.listLocationChangeRequests = listLocationChangeRequests;
exports.reviewLocationChangeRequest = reviewLocationChangeRequest;
const prisma_1 = require("../config/prisma");
const fileUrl_1 = require("../utils/fileUrl");
async function submitLocationChange(req, res) {
    try {
        const { customerId, newFullAddress, newCity, newDistrict, newState, newPincode, newResidenceStatus, proofType, reason, } = req.body;
        if (!customerId) {
        ...
        const proofDocUrl = files ? (0, fileUrl_1.buildFileUrl)(files, "proofDocument") : null;
        if (!proofDocUrl) {
        ...
        const request = await prisma_1.prisma.locationChangeRequest.create({
            data: {
                customerId,
                oldFullAddress,
                oldCity,
...
async function listLocationChangeRequests(req, res) {
    try {
        const requests = await prisma_1.prisma.locationChangeRequest.findMany({
            include: {
                customer: {
                    select: {
                        fullName: true,
...
async function reviewLocationChangeRequest(req, res) {
    try {
        ...
        const request = await prisma_1.prisma.locationChangeRequest.findUnique({
            where: { id },
        ...
        if (action === "APPROVE") {
            await prisma_1.prisma.$transaction([
                prisma_1.prisma.locationChangeRequest.update({
                    where: { id },
            ...
            await prisma_1.prisma.locationChangeRequest.update({
                where: { id },
...
//# sourceMappingURL=locationChange.controller.js.map