...
function termsForInvoiceType(type) {
...
async function renderInvoicePdf(invoice, customer) {
    ...
    const field = (x, yPos, label, value, width, align = "left") => {
    ...
    const footerLines = 1 /* legalName */ + company_1.company.address.length + 1 /* GSTIN */;
    ...
    let fy = footerY + 8;
    doc.text(company_1.company.legalName, margin, fy);
    fy += 9.5;
    company_1.company.address.forEach((line) => {
        doc.text(line, margin, fy);
        fy += 9.5;
    });
    doc.text(`GSTIN: ${company_1.company.gstin}`, margin, fy);
    let cfy = footerY + 8;
    const contactX = pageWidth - margin - 220;
    doc.text(`Email: ${company_1.company.email}`, contactX, cfy, { width: 220, align: "right" });
    cfy += 9.5;
    doc.text(`Support: ${company_1.company.supportEmail}`, contactX, cfy, { width: 220, align: "right" });
    cfy += 9.5;
    doc.text(`Website: ${company_1.company.website}`, contactX, cfy, { width: 220, align: "right" });
    cfy += 9.5;
    doc.text(`WhatsApp: ${company_1.company.whatsapp}`, contactX, cfy, { width: 220, align: "right" });
    doc.end();
    return done;
}
//# sourceMappingURL=invoicePdf.service.js.map